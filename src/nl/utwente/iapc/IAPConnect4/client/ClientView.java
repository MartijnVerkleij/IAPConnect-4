...
public class ClientView extends JFrame {
	...
	private void init() {
		...
		// Element declaration
		titleLabel = new JLabel(controller.getClient().getHandler()
				.getGamePlayers().get(0) + " versus "
				+ controller.getClient().getHandler().getGamePlayers().get(1));
		...
		final JButton aiButton = new JButton("AI Toggled: "
						+ controller.aiIsEnabled());
	...
	private void drawBoard(JPanel target) {
		target.setLayout(new GridLayout(0, controller.getBoard()
						.getBoardWidth()));
		...
		for (int y = 0; y < controller.getBoard().getBoardHeight(); y++) {
			for (int x = 0; x < controller.getBoard().getBoardWidth(); x++) {
				...
				switch (controller.getBoard().getField(x, y)) {
					case 0:
						field.setBackground(empty);
						break;
					case 1:
						field.setBackground(player1);
						break;
					case 2:
						field.setBackground(player2);
						break;
				}
	...
	private void doMove(java.awt.event.ActionEvent e) {
		...
		controller.doMove(boardJPanel.getComponentZOrder((JButton) e
						.getSource()) % controller.getBoard().getBoardWidth());
	}
	...
	public void updateCell(int x, int y, int player, boolean enabled) {
		...
		switch (controller.getBoard().getField(x, y)) {
			case 0:
				...
				break;
			case 1:
				field.setForeground(player1);
				break;
			case 2:
				field.setForeground(player2);
				break;
		}
	}
	...
	public void gameEnd(String winnerName) {
		...
		if (option == JOptionPane.YES_OPTION) {
			...
			try {
				controller.getClient().getHandler()
